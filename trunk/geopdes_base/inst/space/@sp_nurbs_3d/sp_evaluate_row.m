...
W = repmat (reshape (W, 1, space.nsh_max, nel_row), [msh.nqn, 1, 1]);
shape_functions = W.* shp_u .* shp_v .* shp_w;
D = repmat (reshape (sum (shape_functions, 2), msh.nqn, 1, nel_row), [1, space.nsh_max, 1]);
...
if (gradient)
  ...
  Du = repmat (reshape (sum (Bu, 2), msh.nqn, 1, nel_row), [1, sp.nsh_max, 1]);
  Dv = repmat (reshape (sum (Bv, 2), msh.nqn, 1, nel_row), [1, sp.nsh_max, 1]);
  Dw = repmat (reshape (sum (Bw, 2), msh.nqn, 1, nel_row), [1, sp.nsh_max, 1]);

  shape_fun_grads(1,:,:,:) = (Bu - shape_functions .* Du)./D;
  shape_fun_grads(2,:,:,:) = (Bv - shape_functions .* Dv)./D;
  shape_fun_grads(3,:,:,:) = (Bw - shape_functions .* Dw)./D;
