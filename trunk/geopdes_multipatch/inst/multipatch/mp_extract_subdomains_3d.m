% MP_EXTRACT_SUBDOMAINS_3D: create a multipatch geometry extracting a
% set of subdomains from a gigen multipatch geometry.
%
% [new_geometry, new_interfaces, new_boundaries] = mp_extract_subdomains_3d (geometry, interfaces, boundaries, subdomains)
%
% INPUT :
%
...
function [new_geometry, new_interfaces, new_boundaries] = ...
     mp_extract_subdomains_3d (geometry, interfaces, boundaries, subdomains)

% Remove subdomains from the geometry structure
npatch = numel (geometry);
nsubdomains = numel (subdomains);

new_subdomains = zeros (1, npatch);
new_subdomains(subdomains) = 1:nsubdomains;

new_geometry = geometry(subdomains);

% Check which interfaces remain in the new geometry
...
% Check which boundaries remain in the new geometry
keep_bnd = [];
...
added_bnd = nbnd+(1:naddbnd);
if (naddbnd)
  [new_boundaries(added_bnd).name]     = deal (interfaces(int_2_bnd_1).ref);
  [new_boundaries(added_bnd).nsides]   = deal (1);
  new_num  = num2cell (new_subdomains([interfaces(int_2_bnd_1).patch1]));
  [new_boundaries(added_bnd).patches]  = deal (new_num{:});
  [new_boundaries(added_bnd).faces]    = deal (interfaces(int_2_bnd_1).side1);
  for iii = added_bnd
...
added_bnd = nbnd+(1:naddbnd);
if (naddbnd)
  [new_boundaries(added_bnd).name]     = deal (interfaces(int_2_bnd_2).ref);
  [new_boundaries(added_bnd).nsides]   = deal (1);
  new_num  = num2cell (new_subdomains([interfaces(int_2_bnd_2).patch2]));
  [new_boundaries(added_bnd).patches]  = deal (new_num{:});
  [new_boundaries(added_bnd).faces]    = deal (interfaces(int_2_bnd_2).side2);
  [new_boundaries(added_bnd).flag]     = deal (interfaces(int_2_bnd_2).flag);
  [new_boundaries(added_bnd).ornt1]    = deal (interfaces(int_2_bnd_2).ornt1);
  [new_boundaries(added_bnd).ornt2]    = deal (interfaces(int_2_bnd_2).ornt2);
end
end
