...
% INPUTS:
%
%     sp:       space object (see sp_vector_3d_piola_transform)
...
function sp_bnd = sp_eval_boundary_side (sp, msh_side)
  ...
  shape_funs = zeros (3, msh_side.nqn, sp_bnd.nsh_max, msh_side.nel);
  ...
  shape_funs(3,:, sp_bnd1.nsh_max+sp_bnd2.nsh_max+(1:sp_bnd3.nsh_max),:) = sp_bnd3.shape_functions;
  ...
  for ii=1:sp_bnd.nsh_max
    ...
    sp_bnd.shape_functions(3,:,ii,:) = reshape (shape_funs(3,:,ii,:), size (jacdet))./jacdet;
  end
