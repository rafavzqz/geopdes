...
% INPUT:
%
%    space:   class defining the space of discrete functions (see sp_bspline_3d)
...
function [errh1, errl2] = sp_h1_error (sp, msh, u, uex, graduex);
  ...
  grad_valu = zeros (3, msh.nqn, nel_col);
  for iel = 1:msh.nelu
    ...
    grad_valu(1,:,:) = sum (weight .* reshape (sp_col.shape_function_gradients(1,:,:,:), msh.nqn, sp_col.nsh_max, nel_col), 2);
    grad_valu(2,:,:) = sum (weight .* reshape (sp_col.shape_function_gradients(2,:,:,:), msh.nqn, sp_col.nsh_max, nel_col), 2);
    grad_valu(3,:,:) = sum (weight .* reshape (sp_col.shape_function_gradients(3,:,:,:), msh.nqn, sp_col.nsh_max, nel_col), 2);
