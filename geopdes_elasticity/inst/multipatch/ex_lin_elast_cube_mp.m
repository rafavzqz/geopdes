...
fx = @(x, y, z) problem_data.mu(x, y, z) .* (2*cos(x) - 4*x.*y.^2.*z) + ...
                problem_data.lam(x, y, z) .* (cos(x) - 4*x.*y.^2.*z);
fy = @(x, y, z) problem_data.mu(x, y, z) .* (2*sin(y).*z - 4*x.^2.*y.*z) + ...
                problem_data.lam(x, y, z) .* (z.*sin(y) - 4*x.^2.*y.*z);
fz = @(x, y, z) problem_data.mu(x, y, z) .* (-2*(y.*z).^2 - 2*(x.*z).^2 - cos(y) - 4*(x.*y).^2) - ...
                problem_data.lam(x, y, z) .* (2*(x.*y).^2 + cos(y));
