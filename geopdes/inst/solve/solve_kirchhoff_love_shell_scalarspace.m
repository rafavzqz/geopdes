...
function [geometry, msh, space, u] = solve_kirchhoff_love_shell_scalarspace (problem_data, method_data)
...
for iside = 1:numel(drchlt_sides)
  ...
  scalar_dofs = space.boundary(side).dofs;
  for icomp = components
    drchlt_dofs = union (drchlt_dofs, (icomp-1)*space.ndof + scalar_dofs);
  end
end
