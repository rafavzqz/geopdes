...
% 
%  - \int_{Gamma_D} mu*{(\grad u)n v - (\grad v)n u + (Cpen /  he) * (u v)}
%
% and in the right hand-side
%
%  - \int_{Gamma_D} mu*{(\grad v)n g + (Cpen / he) * (v g)}
%
...
function [A, rhs] = sp_weak_drchlt_bc_laplace (space, msh, bnd_sides, bnd_func, coeff, Cpen)

  if (nargin < 6 || isempty (Cpen))
    Cpen = 10 * (min (space.degree) + 1); 
  end
