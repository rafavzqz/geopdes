% SP_EVAL_VECTOR: Compute the value or the derivatives of a function, given by its degrees of freedom, at a given set of points.
%  It uses a scalar-valued space, with a vector of degrees of freedom of length rdim*space.ndof
%
%   [eu, F] = sp_eval_vector (u, space, geometry, pts, [option], [lambda_lame, mu_lame]);
%   [eu, F] = sp_eval_vector (u, space, geometry, npts, [option], [lambda_lame, mu_lame]);
%
% INPUT:
%     
%     u:           vector of dof weights (of length rdim*space.ndof)
...
%     option:      cell array with the fields to plot
%                   accepted options are 'value' (default), 'gradient', 'curl', 'divergence' and 'stress'
...
% Copyright (C) 2011, 2012, 2014, 2015, 2018, 2022 Rafael Vazquez
%
...
function [eu, F] = sp_eval_vector (u, space, geometry, npts, options, lambda_lame, mu_lame)
...
%  end
  ...
  for iopt = 1:nopts
    switch (lower (options{iopt}))
      case 'value'
        eu{iopt} = zeros (ncomp, msh.nqn, msh.nel);
        eunum{iopt} = {1:ncomp, 1:msh.nqn};
        eusize{iopt} = [ncomp, npts];
        value = true;

      case 'gradient'
        eu{iopt} = zeros (ncomp, msh.rdim, msh.nqn, msh.nel);
        eunum{iopt} = {1:ncomp, 1:msh.rdim, 1:msh.nqn};
        eusize{iopt} = [ncomp, msh.rdim, npts];
        grad = true;
