...
function [err_stress] = sp_stress_error_mixed (space, space_p, msh, u, press, lambda_lame, mu_lame, graduex)
% Graduex is the exact gradient. I compute the exact stress using it.
  ...
  for iel = 1:msh.nel_dir(1)
    ...
    spp_col = sp_evaluate_col (space_p, msh_col, 'value', true);
    
    err_h1s = sp_stress_error_mixed (sp_col, spp_col, msh_col, u, press, lambda_lame, mu_lame, graduex);    
    errh1s = errh1s + err_h1s.^2;
  end
  
  err_stress = sqrt (errh1s);
