% OP_GRADSYMU_GRADSYMV_TP: assemble the stiffness matrix A = [a(i,j)], a(i,j) = (epsilon gradsym u_j, gradsym v_i), exploiting the tensor product structure.
%
%   mat = op_gradsymu_gradsymv_tp (spu, spv, msh, [epsilon]);
%   [rows, cols, values] = op_gradsymu_gradsymv_tp (spu, spv, msh, [epsilon]);
%
% INPUT:
%
...
% Copyright (C) 2016, 2017, 2020 Rafael Vazquez
%
...
function varargout = op_gradsymu_gradsymv_tp (space1, space2, msh, coeff)
  ...
  for iel = 1:msh.nel_dir(1)
    ...
    A = A + op_gradsymu_gradsymv (sp1_col, sp2_col, msh_col, coeffs);
  end
