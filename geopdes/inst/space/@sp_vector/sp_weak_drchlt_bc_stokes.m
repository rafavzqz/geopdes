...
%
%
%   [N_mat, N_rhs] = sp_weak_drchlt_bc_stokes (space, msh, bnd_sides, bnd_func, coeff, Cpen)
%
% INPUTS:
%     
...
function [A, rhs] = sp_weak_drchlt_bc_stokes (space, msh, bnd_sides, bnd_func, coeff, Cpen)
...
% Compute the matrices to impose the tangential boundary condition weakly
  for iside = bnd_sides
    ...
    % Second derivative required for Piola transform.
    msh_side_from_interior = msh_precompute (msh_side_from_interior, 'geo_map_der2', true);
