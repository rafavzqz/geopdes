...
function [errl2lap, errl2lap_elem] = sp_l2_lap_error (sp, msh, u, lapuex)
  ...
  errl2lap_elem = sum (reshape (sum ((lap_valu - lap_valex).^2, 1), [msh.nqn, msh.nel]) .* w);

  errl2lap  = sqrt (sum (errl2lap_elem));
  errl2lap_elem  = sqrt (errl2lap_elem);
