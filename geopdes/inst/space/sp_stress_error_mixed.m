...
function [err_stress, err_stress_elem] = sp_stress_error_mixed (sp, spp, msh, u, press, lambda_lame, mu_lame, graduex)
  ...
  gradt_valu = permute (grad_valu, [2 1 3 4]);
  press_valu = sp_eval_msh (press, spp, msh, 'value');

  stress1 = bsxfun (@times, coeff_mu, (grad_valu + gradt_valu));
  for ii = 1:msh.rdim
    stress2(ii,ii,:,:) = press_valu;
  end
  stress_valu = stress1 + stress2;
