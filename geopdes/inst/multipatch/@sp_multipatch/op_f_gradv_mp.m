% OP_F_GRADV_MP: assemble the right-hand side vector r = [r(i)], with  r(i) = (f, grad v_i), in a multipatch domain.
%
%   rhs = op_f_gradv_mp (spv, msh, coeff, [patches]);
%
% INPUT:
%     
...
function rhs = op_f_gradv_mp (space, msh, coeff, patch_list)
  ...
  if (space.npatch ~= msh.npatch)
    error ('op_f_gradv_mp: the number of patches does not coincide')
  end
  
  rhs = zeros (space.ndof, 1);
  for iptc = patch_list
    rhs_loc = op_f_gradv_tp (space.sp_patch{iptc}, msh.msh_patch{iptc}, coeff);
