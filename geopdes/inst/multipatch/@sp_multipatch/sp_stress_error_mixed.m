...
function [err_stress] = sp_stress_error_mixed (space, space_press, msh, u, press, lambda_lame, mu_lame, graduex)
% Graduex is the exact gradient. I compute the exact stress using it.
  ...
  for iptc = 1:msh.npatch
    ...
    press_ptc = press(space_press.gnum{iptc});
    [error_stress(iptc)] = ...
        sp_stress_error_mixed (space.sp_patch{iptc}, space_press.sp_patch{iptc}, msh.msh_patch{iptc}, u_ptc, press_ptc, lambda_lame, mu_lame, graduex);
  end
