...
function rhs = op_f_curlv_mp (space, msh, coeff, patch_list)
  ...
  if (space.npatch ~= msh.npatch)
    error ('op_f_curlv_mp: the number of patches does not coincide')
  end
  
  rhs = zeros (space.ndof, 1);
  for iptc = patch_list
    rhs_loc = op_f_curlv_tp (space.sp_patch{iptc}, msh.msh_patch{iptc}, coeff);
