% SP_EVALUATE_ELEMENT_LIST_PARAM__: compute the basis functions in a given
%   list of elements, in the parametric domain. To be used with care: the
%   parametric domain does not have an intuitive meaning in the multipatch case.
%
%     sp = sp_evaluate_element_list_param__ (space, msh_elems, 'option1', value1, ...)
%
% INPUT:
%     
...
function sp = sp_evaluate_element_list_param__ (space, msh, varargin)
...
%     msh_patch = msh_evaluate_element_list (msh.msh_patch{iptc}, msh.elem_list_of_patch{iptc}); % Not working anymore
    ...
    sp_patch = sp_evaluate_element_list_param (space.sp_patch{iptc}, msh_patch, varargin{:});
    ...
    shape_fun_grads = zeros (msh.ndim, msh_patch.nqn, sp_patch.nsh_max, msh_patch.nel);
    shape_fun_hess = zeros (msh.ndim, msh.ndim, msh_patch.nqn, sp_patch.nsh_max, msh_patch.nel);
