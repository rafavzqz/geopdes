% OP_LAPLACEU_LAPLACEV_MP: assemble the matrix A = [a(i,j)], a(i,j) = (epsilon laplace u_j, laplace v_i), in a multipatch domain.
%
%   mat = op_laplaceu_laplacev_mp (spu, spv, msh, [coeff], [patches]);
%
% INPUT:
%
...
function A = op_laplaceu_laplacev_mp (spu, spv, msh, coeff, patch_list)
  ...
  if ((spu.npatch ~= spv.npatch) || (spu.npatch ~= msh.npatch))
    error ('op_laplaceu_laplacev_mp: the number of patches does not coincide')
  end
  ...
  for iptc = patch_list
    if (nargin < 4 || isempty (coeff))
      Ap = op_laplaceu_laplacev_tp (spu.sp_patch{iptc}, spv.sp_patch{iptc}, msh.msh_patch{iptc});
    else
