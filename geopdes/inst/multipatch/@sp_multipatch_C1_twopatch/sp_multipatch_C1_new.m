...
for kver=1:numel(vertices)
    nu=numel(vertices(kver).patches);
    ...
    for im=1:nu+1 %cycle over all the interfaces containing the vertex (#interfaces=#patches+1, where the last one coincides with the first if the vertex is interior)
        inter=vertices(kver).interface(im).glob_ind; %global index of the interface
        patch_ind1=vertices(kver).interface(im).patch_i1; %global index of left patch of im-th interface
        patch_ind2=vertices(kver).interface(im).patch_i2; %global index of right patch of im-th interface
        vertex_ind1=vertices(kver).interface(im).vertex1; %local index of vertex in left patch
        ...
        switch vertex_ind1 %d0 and D0p to be added in other cases (16 cases in total) ...to be moved where we compute edge functions matrices?
            case 1 %vertex (0,0)
                t0(im,:)=squeeze(derivatives1{vertices(kver).interface(im).patch_i1}(:,2,1));
                t0p(im,:)=squeeze(derivatives2{vertices(kver).interface(im).patch_i1}(:,2,2,1));
                d0(im,:)=(squeeze(derivatives1{vertices(kver).interface(im).patch_i1}(:,1,1)-(all_beta0(inter,1)*(1-0)...
                    +all_beta1(inter,1)*0)*derivatives1{vertices(kver).interface(im).patch_i1}(:,2,1)))...
                    /(all_alpha0(inter,1)*(1-0)+all_alpha0(inter,1)*0);
                d0p(im,:)=((all_alpha0(inter,1)*(1-0)+all_alpha0(inter,1)*0)*squeeze(derivatives1{vertices(kver).interface(im).patch_i1}(:,1,1)-(all_beta0(inter,1)*(1-0)...
                    +all_beta1(inter,1)*0)*derivatives1{vertices(kver).interface(im).patch_i1}(:,2,1))...
                    +(all_alpha0(inter,1)-all_alpha1(inter,1))*squeeze(derivatives2{vertices(kver).interface(im).patch_i1}(:,1,2,1)-(all_beta0(inter,1)*(1-0)...
                    +all_beta1(inter,1)*0)*derivatives2{vertices(kver).interface(im).patch_i1}(:,2,2,1)...
                    +(all_beta0(inter,1)-all_beta1(inter,1))*derivatives1{vertices(kver).interface(im).patch_i1}(:,2,1)))...
                    /((-all_alpha0(inter,1)+all_alpha1(inter,1))^2);
                mix_der2(im,:)=derivatives2{vertices(kver).interface(im).patch_i1}(:,1,2,1);
    ...
    sigma=0;
    for im=1:nu
        vertex_ind=vertices(kver).patches(im).vertex; %local index of the vertex in im-th pacth
        patch_ind=vertices(kver).patches(im).patch_i; %global index of the im-th patch
        switch vertex_ind
            case 1 %vertex (0,0)
                sigma=sigma+norm([derivatives1{vertices(kver).patches(im).patch_i}(1,1,1) derivatives1{vertices(kver).patches(im).patch_i}(2,2,1)]);
            case 2 %vertex (1,0)
                sigma=sigma+norm([derivatives1{vertices(kver).patches(im).patch_i}(1,1,3) derivatives1{vertices(kver).patches(im).patch_i}(2,2,3)]);
            case 3 %vertex (1,1)
                sigma=sigma+norm([derivatives1{vertices(kver).patches(im).patch_i}(1,1,4) derivatives1{vertices(kver).patches(im).patch_i}(2,2,4)]);
            case 4 %vertex (0,1)
                sigma=sigma+norm([derivatives1{vertices(kver).patches(im).patch_i}(1,1,2) derivatives1{vertices(kver).patches(im).patch_i}(2,2,2)]);
        end
    end
    ...
    for im=1:nu
        ...
        for j1=0:2
            for j2=0:2-j1
                ...
                side1=interfaces(vertices(kver).interface(im).patch_i1).side1; %side of left patch
                switch side1
                    case 1
                    V{im,kver}([1, 2, n2+1, n2+2],j)=[d00, d00+d10_b/(p*(k+1)), d00+d10_a/(p*(k+1)),...
                                                  d00+ (d10_a+d10_b+d11_c/(p*(k+1)))/(p*(k+1))]'; %correct that some of these are the already computed ds?
                    case 2
                    V{im,kver}([1, 2, n2+1, n2+2],j)=[d00, d00+d10_b/(p*(k+1)), d00+d10_a/(p*(k+1)),...
