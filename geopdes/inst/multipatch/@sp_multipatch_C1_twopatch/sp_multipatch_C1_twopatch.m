...
function sp = sp_multipatch_C1_twopatch (spaces, msh, geometry, interfaces, boundary_interfaces)
...
%   sp.boundary_orientation = [];
  ...
  sp = class (sp, 'sp_multipatch_C1_twopatch');
...
% I don't understand why we don't need it in the previous system.
      ...
      val = val_grad* (tau1 / degu);  %WARNING: WE DIVIDED BY tau1/degu, which REQUIRES A SMALL MODIFICATION IN THE REF MASK (ADD MULT. BY 1/2) 
      for jj = 1:msh_side(ii).nel
