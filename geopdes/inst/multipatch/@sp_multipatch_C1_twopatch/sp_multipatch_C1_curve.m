...
%       error ('For now, only bilinear patches are implemented')   %da   scommentare!
%     end
...
% Absolute value of the determinant, to make it work for arbitrary orientation -> IMPORTANT!
%       geo_map_jac = msh_side_int{ii}.geo_map_jac;  %da scommentare!
...
%       beta{ii} = numerator ./ denominator; 
        if ii==1
            alpha{ii}=113*(41 + 18*grev_pts{2}')/6544; 
            %-113*(41 + 18*tt{2})/6544;
            beta{ii}=6622447/4826200 - (249660873*grev_pts{2}')/98937100;
            %6622447/4826200 - (249660873*tt{2})/98937100;
        elseif ii==2
            alpha{ii}=339*(17 + 8*grev_pts{2}')/6544; 
            %339*(17 + 8*tt{2})/6544;
            beta{ii}=-225776957/197874200 + (54357979*grev_pts{2}')/24734275;  
            %-225776957/197874200 + (54357979*tt{2})/24734275;
        end
%         alpha{ii}(:)
%         beta{ii}(:)
%         pause
