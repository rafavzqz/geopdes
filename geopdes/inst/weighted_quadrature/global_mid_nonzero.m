...
q_first_el = linspace(distinct_knots(1),distinct_knots(2),degree+2); % punti di quadratura nel primo elemento
q_last_el = linspace(distinct_knots(end-1),distinct_knots(end),degree+2);  % punti di quadratura nell'ultimo elemento
...
for i = 1:ndof
    ...
    if (i == 1) % gestisco a parte la prima e l'ultima funzione di base. Qui lavoro con la prima
        quad_points = linspace(distinct_local_knots(1),distinct_local_knots(2),degree+2);
        quad_points = quad_points(1:degree+1);
    elseif (i == ndof) % Qui con l'ultima
        quad_points = linspace(distinct_local_knots(1),distinct_local_knots(2),degree+2);
        quad_points = quad_points(2:degree+2);
    else
        if distinct_local_knots(1) == 0 % se la funzione ha supporto nel primo elemento, prendo i p+2 punti di quadratura corrispondenti
            quad_points = linspace(distinct_local_knots(1),distinct_local_knots(2),degree+2);
        else
        ...
        if distinct_local_knots(end) == 1 % se la funzione ha supporto nell'ultimo elemento, prendo i p+2 punti di quadratura corrispondenti
	...
	Q.quad_points{i} = unique(quad_points);
