...
function varargout = op_laplaceu_laplacev (spu, spv, msh, coeff)
  ...
  for iel = 1:msh.nel
    if (all (msh.jacdet(:,iel)))
      shpu_iel = reshape (laplaceu(:, :, 1:spu.nsh(iel), iel), spu.ncomp, msh.nqn, 1, spu.nsh(iel));
%       shpu_iel = repmat (shpu_iel, [1,1,spv.nsh(iel),1]);
      shpv_iel = reshape (laplacev(:, :, 1:spv.nsh(iel), iel), spv.ncomp, msh.nqn, spv.nsh(iel), 1);
