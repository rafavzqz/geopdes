% OP_GRADV_N_LAPLACEU: assemble the matrix A = [a(i,j)], a(i,j) = (epsilon (grad v n)_j, Delta u_i), with n the normal vector.
%
...
% Copyright (C) 2014, 2017, 2023 Rafael Vazquez
% Copyright (C) 2023 Michele Torre
%
...
function varargout = op_gradv_n_laplaceu (spu, spv, msh, coeff)
  ...
  for iel = 1:msh.nel
    if (all (msh.jacdet(:,iel)))
      ...
      warning ('geopdes:jacdet_zero_at_quad_node', 'op_gradv_n_laplaceu: singular map in element number %d', iel)
    end
  end
  ...
  elseif (nargout == 3)
    ...
    error ('op_gradv_n_laplaceu: wrong number of output arguments')
  end
