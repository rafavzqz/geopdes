...
% INPUTS:
%
%     sp:       space object (see sp_vector_2d_curl_transform)
...
% Fields for the transformation
  ...
  switch (iside)
   case {1, 2}
     sp_bnd.nsh = sp_bnd2.nsh;
     sp_bnd.connectivity = sp_bnd2.connectivity;

     sp_bnd.shape_functions = zeros (2, msh_side.nqn, sp_bnd.nsh_max, msh_side.nel);
     sp_bnd.shape_functions(2,:,:,:) = sp_bnd2.shape_functions;
     sp_bnd.shape_functions = geopdes_prod__ (JinvT, sp_bnd.shape_functions);
   case {3, 4}
     sp_bnd.nsh = sp_bnd1.nsh;
     sp_bnd.connectivity = sp_bnd1.connectivity;

     sp_bnd.shape_functions = zeros (2, msh_side.nqn, sp_bnd.nsh_max, msh_side.nel);
     sp_bnd.shape_functions(1,:,:,:) = sp_bnd1.shape_functions;
     sp_bnd.shape_functions = geopdes_prod__ (JinvT, sp_bnd.shape_functions);
  end
