% SP_CURL_TRANSFORM_2D: map a function space to the physical domain with a curl conserving transform.
%
%     sp = sp_curl_transform_2d (sp, msh)
%
% INPUTS:
%
%     sp:  space structure defined in the parametric domain (see sp_bspline_2d_param, sp_bspline_curl_transform_2d)  
...
function sp = sp_curl_transform_2d (sp, msh)

  sp = do_sp_curl_transform_2d__ (sp, msh);

  if (isfield (msh, 'boundary'))
    for iside = 1:numel(msh.boundary)
      sp.boundary (iside) = do_sp_curl_transform_2d__ (sp.boundary(iside), msh.boundary(iside));
    end
  end
  ...
  if (isfield (sp, 'shape_function_curls'))
    for ii=1:sp.nsh_max
      sp.shape_function_curls(:,ii,:) = ...
         squeeze (sp.shape_function_curls(:,ii,:))./jacdet;
    end
  end

  if (isfield (sp, 'shape_function_gradients'))
    error ('sp_curl_transform_2d: the computation of the gradients with the curl transform is not allowed yet')
  end

  if (isfield (sp, 'shape_function_divs'))
    error ('sp_curl_transform_2d: the computation of the divergence with the curl transform is not allowed yet')
  end
