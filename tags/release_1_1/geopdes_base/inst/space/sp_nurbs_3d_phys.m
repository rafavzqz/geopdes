% SP_NURBS_3D_PHYS: Construct a tensor-product space of NURBS on the physical domain in 3D.
%
%     sp = sp_nurbs_3d_phys (nurbs, msh, ['option', value, ...])
%
% INPUTS:
%
...
function sp = sp_nurbs_3d_phys (nurbs, msh, varargin)

  sp = sp_nurbs_3d_param (nurbs, msh, varargin{:});

  if (isfield (sp, 'shape_function_gradients'))
    JinvT = geopdes_invT__ (msh.geo_map_jac);
    JinvT = reshape (JinvT, [3, 3, msh.nqn, msh.nel]);
    shape_fun_grads = reshape (sp.shape_function_gradients, [3, msh.nqn, sp.nsh_max, msh.nel]);
  sp.shape_function_gradients = geopdes_prod__ (JinvT, shape_fun_grads);
  end

  sp.spfun  = @(MSH) sp_nurbs_3d_phys (nurbs, MSH, varargin{:});
