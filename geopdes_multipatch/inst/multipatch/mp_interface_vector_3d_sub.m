% MP_INTERFACE_VECTOR_3D_SUB: create a global numbering of vectorial basis functions in three-dimensional multipatch geometries.
%
...
function [glob_num, glob_ndof, glob_num_bnd] = mp_interface_vector_3d_sub (interfaces, boundaries, sp)
...
% Reorder the sides, to avoid conflicts in setting the numbering
  ...
  for int = 1:numel (boundaries)
    for iside = 1:boundaries(int).nsides;
      iptc = boundaries(int).patches(iside);
      face = boundaries(int).faces(iside);

      bnd_dofs = sp{iptc}.boundary(face).dofs;

      bnd_dofs_comp1 = reshape (sp{iptc}.boundary(face).comp_dofs{1}, ...
                    sp{iptc}.boundary(face).ndof_dir(1,:));
      bnd_dofs_comp2 = reshape (sp{iptc}.boundary(face).comp_dofs{2}, ...
                    sp{iptc}.boundary(face).ndof_dir(2,:));
      bnd_dofs_comp3 = reshape (sp{iptc}.boundary(face).comp_dofs{3}, ...
                    sp{iptc}.boundary(face).ndof_dir(3,:));

      if (boundaries(int).flag == -1), 'oops'
        bnd_dofs_comp1 = bnd_dofs_comp1';
        bnd_dofs_comp2 = bnd_dofs_comp2';
        bnd_dofs_comp3 = bnd_dofs_comp3';
      end
      if (boundaries(int).ornt1 == -1), 'oops'
        bnd_dofs_comp1 = flipud (bnd_dofs_comp1);
        bnd_dofs_comp2 = flipud (bnd_dofs_comp2);
        bnd_dofs_comp3 = flipud (bnd_dofs_comp3);
      end
      if (boundaries(int).ornt2 == -1), 'oops'
        bnd_dofs_comp1 = fliplr (bnd_dofs_comp1);
        bnd_dofs_comp2 = fliplr (bnd_dofs_comp2);
        bnd_dofs_comp3 = fliplr (bnd_dofs_comp3);
      end

      bnd_dofs_new = [bnd_dofs_comp1(:) bnd_dofs_comp2(:) bnd_dofs_comp3(:)];
      glob_num_bnd{int}{iside} =  glob_num{iptc}(bnd_dofs(:));      
    end
  end
end
