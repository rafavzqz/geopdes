% mpi_gmres: solve Ax=b with parallel gmres
%
% [sol, err] = mpi_gmres (A, b, x0, max_iter, comm)
%
% INPUT:
% 
% A:  matrix
...
function [sol, err] = mpi_gmres(A, b, x0, max_iter, comm)
...
for sol_iter=1:max_iter
  ...
  % note that it can be done this way because V is ortonormal
  coef = V'*v;
  v    = v - V * coef;
  h    = norm(v,2);
  H(:,sol_iter) = coef';
...
%! [sol err]=mpi_gmres(Am,b,x0,dim,comm);
%! sol_serial=A\b;
%! if(cpu_ID==0)
