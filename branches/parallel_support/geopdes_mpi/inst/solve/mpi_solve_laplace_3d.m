% MPI_SOLVE_LAPLACE_3D: Solve a 3d Laplace problem with a B-spline discretization (non-isoparametric approach). 
%
...
function [geometry, msh, space, u] = ...
              mpi_solve_laplace_3d (mpi_comm, problem_data, method_data)
...
[zeta boundary_list] = msh_partition (mpi_size, 'knots',knots,'degree',degree, 'index', mpi_rank+1);

% Construct msh structure
rule     = msh_gauss_nodes (nquad);
[qn, qw] = msh_set_quad_nodes (zeta, rule);
msh      = msh_3d_tensor_product (zeta, qn, qw, 'boundary', boundary_list);
...
for iside = intersect(nmnn_sides, msh.boundary_list)
