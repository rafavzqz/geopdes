...
function sp = bsp_2_nrb_1d__ (sp, msh, W)
  ...
  if (isfield (sp, 'shape_function_gradients') && ~isempty (sp.shape_function_gradients))
    Bu = W .* squeeze (sp.shape_function_gradients);
    Du = repmat (reshape (sum (Bu, 2), msh.nqn, 1, msh.nel), [1, sp.nsh_max, 1]);
    Nu = (Bu - sp.shape_functions .* Du)./D;
    sp.shape_function_gradients = Nu;
  end
