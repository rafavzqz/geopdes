% SP_BSPLINE_TH_3D_PHYS: Construct the Taylor-Hood pair of B-Splines spaces on the physical domain in 3D.
%
...
function [spv, spp] = sp_bspline_th_3d_phys (knotsp, degree, msh)

  for idim = 1:3
    knotsv{idim}  = sort ([knotsp{idim}, unique(knotsp{idim})]);
  end

spp       = sp_bspline_3d_phys (knotsp, degree, msh, 'gradient', false);
...
function spv = do_spv (knotsv, degree, msh)
  spv  = sp_bspline_3d_phys (knotsv, degree+1, msh);
  spv  = sp_scalar_to_vector_3d (spv, spv, spv, msh, 'divergence', true);
end
