% MP_SP_TO_VTK_2D: Export multipatch results to VTK format for plotting.
%
%  mp_sp_to_vtk_2d (u, space, geometry, gnum, npts, filename, fieldname)
%
% INPUT:
%     
...
function mp_sp_to_vtk_2d (u, space, geometry, gnum, npts, filename, fieldname)
...
'</VTKFile> \n');
  ...
  if (fid < 0)
    error ('mp_sp_to_vtk_2d: could not open file %s', pvd_filename);
  end
  ...
  for iptc = 1:numel(geometry)
    ...
    sp_to_vtk_2d (u(gnum{iptc}), space{iptc}, geometry(iptc), npts, ...
